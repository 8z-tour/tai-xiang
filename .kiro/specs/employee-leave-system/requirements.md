...
**使用者故事:** 作為管理者，我想要在權限資料管理中按工號篩選用戶，以便快速找到特定員工的資料進行管理

#### 驗收標準

1. WHEN 管理者存取請假管理頁面的權限資料區塊 THEN 員工請假系統 SHALL 在用戶列表上方顯示工號篩選下拉選單
2. WHEN 管理者點選工號篩選下拉選單 THEN 員工請假系統 SHALL 顯示所有現有用戶的工號選項以及"全部"選項
3. WHEN 管理者選擇特定工號 THEN 員工請假系統 SHALL 僅顯示該工號對應的用戶資料
4. WHEN 管理者選擇"全部"選項 THEN 員工請假系統 SHALL 顯示所有用戶的完整列表
5. WHEN 管理者變更篩選條件 THEN 員工請假系統 SHALL 即時更新顯示的用戶列表
6. WHEN 篩選條件生效時 THEN 員工請假系統 SHALL 在桌面版表格和手機版卡片列表中都套用相同的篩選邏輯
7. WHEN 管理者新增或刪除用戶後 THEN 員工請假系統 SHALL 自動更新篩選下拉選單的選項列表

### 需求 21

**使用者故事:** 作為員工，我想要在請假紀錄頁面看到正確排序的欄位，以便更直觀地查看我的請假資訊

#### 驗收標準

1. WHEN 員工存取請假紀錄頁面的請假記錄表格 THEN 員工請假系統 SHALL 按照以下順序顯示欄位：簽核狀況、開始時間、結束時間、請假時數、假別、事由、申請時間
2. WHEN 請假記錄表格顯示 THEN 員工請假系統 SHALL 確保表格標題欄位與資料欄位的順序完全一致
3. WHEN 請假記錄表格載入資料 THEN 員工請假系統 SHALL 在每個資料列中按照指定順序顯示對應的資料內容