...
- [x] 19. 實作用戶管理工號篩選功能
  - [x] 19.1 建立篩選狀態管理
    - 在UserManagement組件中新增篩選狀態（selectedEmployeeId, filteredUsers）
    - 實作篩選邏輯函數（handleFilterChange）
    - 建立篩選選項生成邏輯（動態從用戶列表生成）
    - _需求: 20.2, 20.3, 20.7_

  - [x] 19.2 實作篩選UI組件
    - 在用戶列表上方新增工號篩選下拉選單
    - 實作響應式佈局（桌面版水平排列，手機版垂直堆疊）
    - 整合篩選下拉選單與現有操作按鈕的佈局
    - _需求: 20.1_

  - [x] 19.3 整合篩選邏輯到顯示組件
    - 更新桌面版表格顯示以使用篩選後的用戶列表
    - 更新手機版卡片列表以使用篩選後的用戶列表
    - 確保篩選邏輯在兩種顯示模式中一致運作
    - _需求: 20.3, 20.6_
  ...
  - [x] 19.5 撰寫工號篩選功能屬性測試
    ...
    - **屬性 38: 跨平台篩選一致性**
    - **屬性 39: 篩選選項同步**
    - **驗證: 需求 20.2, 20.3, 20.5, 20.6, 20.7**

- [x] 20. 工號篩選功能檢查點 - 確保所有測試通過
  - 確保所有測試通過，如有問題請詢問使用者

- [-] 21. 修正請假紀錄表格欄位順序
  - [x] 21.1 調整表格標題欄位順序
    - 修改LeaveRecord.tsx中的表格標題順序為：簽核狀況、開始時間、結束時間、請假時數、假別、事由、申請時間
    - 確保標題文字使用正確的翻譯鍵值
    - _需求: 21.1, 21.2_
